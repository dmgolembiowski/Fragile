...
class CursesMenu(object):
    ...
    def start(self, show_exit_option=None):
        """
        Start the menu in a new thread and 
        allow the user to interact with it.
        
        The thread is a daemon, so 
        
        :meth:
        `join()<cursesmenu.CursesMenu.join>` 
        should be called if there's a possibility
        that the main thread will exit before the menu is done

        :param bool show_exit_option: 
        Whether the exit item should be shown, defaults to\
    ...
    def show(self, show_exit_option=None):
        ...
        :param bool show_exit_option: 
        Whether the exit item should be shown, 
    ...
    def draw(self):
        """
        Redraws the menu and refreshes the screen. 
        Should be called whenever something 
        changes that needs to be redrawn.
        """
    ...
    def wait_for_start(self, timeout=None):
        ...
        :return: False if timeout is given 
            and operation times out, 
            True otherwise. 
        None before Python 2.7
        """
...
def clear_terminal():
    """
    Call the platform specific function to
